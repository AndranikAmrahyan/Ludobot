...
# Импортируем модуль для управления историей чатов
...
async def ludobot_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    clean_log_text = chm.update_chat_history(clean_log_text, chat_id)
    # print(clean_log_text)
    
    try:
        # Передаем очищенный текст в модуль предсказаний
        ocp.LAST_LOG_TEXT = clean_log_text
        ...
        for model in models_to_run:
            try:
                ...
                numbers = ocp.parse_numbers_from_text(clean_log_text)
                n = len(numbers)
        ...
        if color_counts:
            ...
            if consensus_color[1] >= 2:  # Если минимум 2 модели согласны
                if "красное" in consensus_color[0].lower():
                    consensus_emoji = "🔴"
                elif "чёрное" in consensus_color[0].lower():
                    consensus_emoji = "⚫️"
                else:
                    consensus_emoji = "🟢"
                
                # response += f"💡 <b>Консенсус:</b> {consensus_emoji} {consensus_color[0]} ({consensus_color[1]}/3 моделей)\n\n"
                response += f"💡 <b>Рекомендуем:</b> {consensus_color[0]}\n"
        
        response += "📚 Используйте <code>/rec</code> для получения рекомендаций по стратегии"
